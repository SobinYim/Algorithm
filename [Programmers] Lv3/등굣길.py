def solution(m, n, puddles): #m*n 크기의 격자, 물에 잠긴 지역의 위치
    m,n=m+1,n+1
    matrix=[[0]*m for _ in range(n)] #해당 위치로 갈 수 있는 경우의 수
    matrix[0][1]=1 #(1,1)에서 1로 시작하기 위함
    for y in range(1,n):
        for x in range(1,m):
            if [x,y] in puddles:
                continue
            else:
                matrix[y][x]=(matrix[y-1][x]+matrix[y][x-1])%1000000007 #해당 위치로 올 수 있는 경우의 수의 합
    return matrix[-1][-1]
...
matrix[0][1]=1 이 부분은 matrix[1][0]=1 이어도 무방
